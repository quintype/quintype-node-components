 ...
 * @component
 */
export function ImageGalleryElement({
  className,
  imageAspectRatio,
  defaultWidth,
  element,
  ...
  const images = element["story-elements"].map((image, index) => (
    ...
    >
      <ResponsiveImage
        ...
        imgParams={imgParams ? { auto: ["format", "compress"], ...imgParams } : { auto: ["format", "compress"] }}
  ...
  return (
    <div className="story-element-image-gallery" data-test-id="story-element-image-gallery">
      {images}
    </div>
  );
}
